...
class VolumeEstimator:
    ...
    def estimate_volume_in_polygon_roi(self, points: np.ndarray, polygon: List[List[float]], preprocessor: PointCloudPreprocessor = None, cluster_voxel_size: float = 0.1, cluster_min_points_per_component: int = 10) -> Tuple[float, int]:
        if len(points) == 0:
            return 0.0, 0
        ...
        try:
            ...
            if preprocessor is not None and len(roi_points) > 0:
                roi_pcd = o3d.geometry.PointCloud()
                roi_pcd.points = o3d.utility.Vector3dVector(roi_points)
                roi_pcd = preprocessor.connectivity_cluster_filter(
    ...
    def save_roi_points_as_pcd(self, points: np.ndarray, polygon: List[List[float]], output_path: str, preprocessor: PointCloudPreprocessor = None, cluster_voxel_size: float = 0.1, cluster_min_points_per_component: int = 10) -> bool:
        try:
            path = MplPath(polygon)
            inside_mask = path.contains_points(points[:, :2])
            inside_mask = np.asarray(inside_mask, dtype=bool)
    ...
    def estimate_volumes_with_rois(self, points: np.ndarray, rois: List[List[List[float]]], preprocessor: PointCloudPreprocessor = None, cluster_voxel_size: float = 0.1, cluster_min_points_per_component: int = 10) -> dict:
        ...
        for idx, poly in enumerate(rois):
            vol, count = self.estimate_volume_in_polygon_roi(
                points, poly, preprocessor=preprocessor,
...
def process(input_path: str, output_path: str = None, grid_size: float = 0.1, rois: str = None, cluster_voxel_size: float = 0.1, cluster_min_points_per_component: int = 10):
    ...
    if rois:
        ...
        for idx, poly in enumerate(roi_data):
            ...
            estimator.save_roi_points_as_pcd(
                points, poly, str(roi_output_path),
                preprocessor=preprocessor,
                cluster_voxel_size=cluster_voxel_size, cluster_min_points_per_component=cluster_min_points_per_component
            )
        
        result = estimator.estimate_volumes_with_rois(
            points, roi_data,
            preprocessor=preprocessor,
            cluster_voxel_size=cluster_voxel_size, cluster_min_points_per_component=cluster_min_points_per_component
        )
...
def main():
    ...
    parser.add_argument("--cluster-voxel-size", type=float, default=0.1, help="连通性聚类体素大小 (默认: 0.1m)")
    parser.add_argument("--cluster-min-points-per-component", type=int, default=10, help="连通域最小点数阈值 (默认: 10)")
    args = parser.parse_args()

    process(args.input, args.output, args.grid_size, args.rois, args.cluster_voxel_size, args.cluster_min_points_per_component)
