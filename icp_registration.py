...
def _compute_default_distance(aligned_cloud: np.ndarray, reference_cloud: np.ndarray) -> float:
...
def cloud_compare_icp(
    ...
    if max_correspondence_distance is None:
        max_correspondence_distance = _compute_default_distance(source, target)
        if verbose:
            print(f"  自动计算对应点距离阈值: {max_correspondence_distance:.6f}")

    # 随机采样（如果点云过大）
    if len(source) > random_sample_limit:
        if verbose:
            print(f"  待配准点云超过限制 ({len(source)} > {random_sample_limit}), 随机采样")
        indices = np.random.choice(len(source), random_sample_limit, replace=False)
        source = source[indices]
    if len(target) > random_sample_limit:
        if verbose:
            print(f"  参考点云超过限制 ({len(target)} > {random_sample_limit}), 随机采样")
    ...
    for iteration in range(max_iterations):
        ...
        if final_src_corr_count < 3:
            if verbose:
                print(f"  迭代 {iteration + 1}: 对应点不足 ({final_src_corr_count}), 停止配准")
            break
        ...
        if verbose:
            print(f"  迭代 {actual_iterations:3d}: RMS = {current_rms:.8f}, "
                  f"ΔRMS = {rms_change:.6e}, 对应点 = {final_src_corr_count}")

        if rms_change < rms_diff and iteration > 0:
            if verbose:
                print(f"  收敛: ΔRMS ({rms_change:.6e}) < RMS difference ({rms_diff})")
            break
...
def icp_registration(aligned_cloud: np.ndarray,
                     reference_cloud: np.ndarray,
    ...
    返回:
        ...
        fitness: 配准适应度
    """
    result, transform, fitness, _ = cloud_compare_icp(
...
def find_correspondences(source_points: np.ndarray, target_points: np.ndarray, threshold: float = 0.02):
...
def compute_rms_difference_transform(source_points: np.ndarray, target_points: np.ndarray,
...
def save_point_cloud(file_path: str, points: np.ndarray) -> None:
    """保存点云数据到文件"""
...
def main():
    parser = argparse.ArgumentParser(
        description="点云ICP配准工具 (CloudCompare Fine Registration 风格)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  # 基本配准，使用默认参数 (RMS difference = 1.0e-5, 20次迭代)
  python icp_registration.py -a aligned.pcd -r reference.pcd

  # 严格收敛要求
  python icp_registration.py -a aligned.pcd -r reference.pcd --rms-diff 1e-8

  # 小重叠率场景 (如部分重叠的扫描)
  python icp_registration.py -a aligned.pcd -r reference.pcd --overlap 0.5

  # 使用点到点模式 (默认是点到面)
  python icp_registration.py -a aligned.pcd -r reference.pcd --point-to-point
        """
    )

    # 文件参数
    parser.add_argument("-a", "--aligned", required=True,
                        help="待配准的点云文件路径 (aligned.pcd)")
    parser.add_argument("-r", "--reference", required=True,
                        help="参考点云文件路径 (reference.pcd)")
    parser.add_argument("-o", "--output",
                        help="配准结果输出路径 (默认自动生成)")

    # CloudCompare 核心参数
    icp_group = parser.add_argument_group("ICP 配准参数 (CloudCompare 风格)")
    icp_group.add_argument("--rms-diff", type=float, default=1e-5,
                           help="RMS difference 收敛阈值 (默认: 1.0e-5)")
    icp_group.add_argument("--max-iterations", type=int, default=20,
                           help="最大迭代次数 (默认: 20)")
    icp_group.add_argument("--overlap", type=float, default=1.0,
                           help="重叠率 0~1 (默认: 1.0)")
    icp_group.add_argument("--point-to-plane", dest="point_to_plane",
                           action="store_true", default=True,
                           help="使用点到面距离 (默认开启)")
    icp_group.add_argument("--point-to-point", dest="point_to_plane",
                           action="store_false",
                           help="使用点到点距离")
    icp_group.add_argument("--random-sample-limit", type=int, default=50000,
                           help="随机采样上限 (默认: 50000)")
    icp_group.add_argument("--correspondence-distance", type=float, default=None,
                           help="对应点搜索最大距离 (默认: 自动根据点云尺度计算)")
    icp_group.add_argument("--threshold", type=float, default=None,
                           help="(已弃用, 等同于 --correspondence-distance)")
    ...
    try:
        print(f"加载待配准点云: {args.aligned}")
        aligned_cloud = load_point_cloud(args.aligned)

        print(f"加载参考点云: {args.reference}")
        reference_cloud = load_point_cloud(args.reference)

        print(f"待配准点云形状: {aligned_cloud.shape}")
        print(f"参考点云形状: {reference_cloud.shape}")
        ...
        print(f"  RMS difference: {args.rms_diff}")
        print(f"  重叠率: {args.overlap}")
        print(f"  随机采样限制: {args.random_sample_limit}")
        if corr_dist is not None:
            print(f"  对应点距离阈值: {corr_dist}")

        # 执行配准
        transformed_points, transform_matrix, fitness, iterations = cloud_compare_icp(
            aligned_cloud, reference_cloud,
        ...
        if len(transformed_points) == len(reference_cloud):
            ...
            if nn_errors:
                ...
                print(f"  平均最近邻配准误差: {avg_error:.6f}m")
        ...
        # o3d.pipelines.registration.icp(
        #     aligned_cloud,
        #     reference_cloud,
        #     max_correspondence_distance=corr_dist,
        #     estimation_method=o3d.pipelines.registration.TransformationEstimationPointToPlane() if args.point_to_plane else o3d.pipelines.registration.TransformationEstimationPointToPoint(),
        #     criteria=o3d.pipelines.registration.ICPConvergenceCriteria()
        # )
        # 对齐点云应用变换
        transformed_aligned_cloud = (rotation @ aligned_cloud.T).T + translation
        print(f"配准点云形状: {transformed_aligned_cloud.shape}")
        
        # 保存变换后的点云
        save_point_cloud(output_path, transformed_aligned_cloud)
