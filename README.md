...
## 点云数据预处理工具 (pcd_deal.py)

### 功能特点

- **异常值移除**：通过在不同投影平面上的网格过滤来移除离群点
- **自动裁剪**：根据多个投影平面的边界自动裁剪点云
- **天花板检测**：自动检测并移除天花板或上部干扰物
- **体积计算**：计算点云的体积
- **ROI区域分析**：支持在特定感兴趣区域(ROI)内计算体积
- **点云预处理**：综合应用多种预处理方法，优化点云质量

### 使用方法

```bash
# 基本预处理，只输出预处理后的点云
python3 pcd_deal.py input.pcd

# 指定输出路径
python3 pcd_deal.py input.pcd -o output.pcd

# 指定体积计算栅格大小
python3 pcd_deal.py input.pcd -o output.pcd -g 0.2

# 在指定ROI区域内计算体积
python3 pcd_deal.py input.pcd -o output.pcd -g 0.1 -r "[[[x1,y1],[x2,y2],[x3,y3],[x4,y1]]]"
```
...
| 参数 | 缩写 | 类型 | 描述 | 默认值 |
|------|------|------|------|--------|
| `input` | - | 字符串 | 输入PCD文件路径（必需） | - |
| `-o`, `--output` | - | 字符串 | 预处理后点云输出路径 | 自动生成 |
| `-g`, `--grid-size` | - | 浮点数 | 体积计算栅格大小（米） | 0.1 |
| `-r`, `--rois` | - | 字符串 | ROI多边形区域定义（JSON字符串） | 无 |

### 预处理流程

1. **投影异常值移除**：
   - 在xz、xy、yz三个平面上分别进行网格化过滤
   - 移除点数少于阈值的网格单元
